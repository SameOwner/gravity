...
InputChecker::InputChecker()
{
	...
	inputList_[Input_Key::R2] = Connect_Key_Pad(KEYCODE::R, PADBUTTON::NUM8);
	inputList_[Input_Key::L2] = Connect_Key_Pad(KEYCODE::TAB, PADBUTTON::NUM7);
...
Vector2 InputChecker::Stick()const
{
	...
	Vector2 stickVector = GamePad::GetInstance().Stick();
...
{
	return GamePad::GetInstance().POVStateDown();
}

int InputChecker::GetPovTriggerDownAngle() const
{
	return GamePad::GetInstance().POVTriggerDown();
}
